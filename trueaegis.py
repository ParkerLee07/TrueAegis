...
def run_selected_mode(validate_mode=False, report_mode=False, pdf_mode=False, quiet_mode=False):
    ...
    netsniper_data = load_json(findings_file)
...
def launch_web_dashboard():
    ...
    try:
        ...
        console.print("[yellow]Install the web launcher or run:[/yellow] python3 ~/TrueAegis/web/app.py")
...
def show_menu():
    while True:
        console.print(
            Panel.fit(
                "[bold cyan]TrueAegis v1.0-beta[/bold cyan]\nExposure Intelligence Tool",
                border_style="cyan"
            )
        )
